...
// Tray icons are generated by electron/prebuild-tray-icons.js from
// the project-root logo.png. In dev they're read from client/build/;
// in packaged builds electron-builder's `buildResources: build` copies
// the same directory to resources/build/, which is process.resourcesPath.
const trayIconDir = app.isPackaged
  ? path.join(process.resourcesPath, 'build')
  : path.join(__dirname, '..', 'build');
let defaultIcon: Electron.NativeImage | null = null;
let alertIcon: Electron.NativeImage | null = null;

function loadTrayIcons() {
  const defaultPath = path.join(trayIconDir, 'tray-default.png');
  const alertPath = path.join(trayIconDir, 'tray-alert.png');
  if (!fs.existsSync(defaultPath)) {
    console.warn(`[tray] missing ${defaultPath} — run electron/prebuild-tray-icons.js`);
  }
  defaultIcon = nativeImage.createFromPath(defaultPath);
  alertIcon = nativeImage.createFromPath(alertPath);
...
function createTray() {
  // Icons are generated at build time by electron/prebuild-tray-icons.js
  // (see package.json prebuild:electron hook) and live in client/build/,
  // which is also electron-builder's buildResources directory.
  loadTrayIcons();
