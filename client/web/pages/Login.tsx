...
import { syncServerConfig } from '../hooks/useServerConfig';
...
declare global {
    interface Window {
        electronAPI?: {
            ...
            updater: { check: () => Promise<any>; download: () => Promise<void>; install: () => Promise<void> };
        };
    }
}
...
export default function Login() {
    ...
    async function loadServerConfig() {
        if (window.electronAPI?.store) {
            ...
            const ws = await window.electronAPI.store.get('wsUrl');
            const upd = await window.electronAPI.store.get('updateUrl');
            if (url) setServerUrl(url);
            if (ws) setWsUrl(ws);
            if (upd) setUpdateUrl(upd);
        } else {
            setServerUrl(localStorage.getItem('serverUrl') || 'http://localhost:8000/api');
            setWsUrl(localStorage.getItem('wsUrl') || 'ws://localhost:8000/ws');
            setUpdateUrl(localStorage.getItem('updateUrl') || '');
        }
    }
    ...
    async function saveServerConfig() {
        ...
        const ws = wsUrl.trim();
        const upd = updateUrl.trim();
        if (window.electronAPI?.store) {
            await window.electronAPI.store.set('serverUrl', url);
            await window.electronAPI.store.set('wsUrl', ws);
            await window.electronAPI.store.set('updateUrl', upd);
        }
        localStorage.setItem('serverUrl', url);
        localStorage.setItem('wsUrl', ws);
        localStorage.setItem('updateUrl', upd);
    ...
    useEffect(() => {
        ...
        // Electron 下 window.location.host 是空（file:// 协议），没有真实域名，跳过
                    ...
                    {tenantSelection && (
                        <div style={{
                            ...
                            <div style={{
                                ...
                                maxWidth: '400px',
                                width: '90%',
                                maxHeight: 'min(620px, calc(100vh - var(--titlebar-height) - 64px))',
