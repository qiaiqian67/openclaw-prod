...
export default function AgentDetailPage() {
    ...
    const ensureSessionSocket = (sess: any, agentId: string, authToken: string) => {
        ...
        // Electron 下 window.location.host 是空（file:// 协议），使用 localStorage 中用户配置的 wsUrl
        // wsUrl 已包含 /ws 前缀（如 ws://server:8000/ws），只需追加 /chat/...
        const wsBase = localStorage.getItem('wsUrl') || `${protocol}//${window.location.host}/ws`;
        ...
        const ws = new WebSocket(`${wsBase}/chat/${agentId}?token=${authToken}${sessionParam}&lang=${lang}`);
                    ...
                    activeTab === 'chat' && (
                        <div
                            ...
                            style={{
                                ...
                                flex: 1,
                                minHeight: 0,
                                height: 'calc(100vh - var(--titlebar-height) - 100px)',
                                margin: '0 8px 8px',
