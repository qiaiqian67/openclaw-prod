...
export default function Layout() {
    ...
    // Tray flash: when there are unread notifications and the window is not
    // focused, ping the main process to start flashing the tray icon. Stops
    // automatically when the window regains focus (handled in main.ts via
    // mainWindow.on('focus', stopTrayFlash)) or when the count drops to 0.
    useEffect(() => {
        if (typeof window === 'undefined' || !window.electronAPI?.tray) return;
        if (unreadCount > 0 && !document.hasFocus()) {
            window.electronAPI.tray.flash();
    ...
    // (`.chat-messages` and the settings form), so resetting the main scroll
    // doesn't disturb their position.
    useEffect(() => {
    ...
    // Tray menu → "打开设置" sends tray:open-settings from main. Treat that
    // as a deep link into the company settings page so the user can land
    // on a real screen instead of just seeing the window come forward.
    useEffect(() => {
        if (!window.electronAPI?.tray?.onOpenSettings) return;
        return window.electronAPI.tray.onOpenSettings(() => {
            navigate('/enterprise');
        });
    }, [navigate]);
