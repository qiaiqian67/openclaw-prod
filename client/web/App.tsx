...
export default function App() {
    ...
    // In Electron, reserve 32px at the top for the custom title bar.
    // The variable is read by CSS in body padding and existing 100dvh/100vh
    // height calculations so layouts don't overflow.
    ...
    if (loading) {
        return (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - var(--titlebar-height))', color: 'var(--text-tertiary)' }}>
                加载中...
            </div>
        );
    }
    ...
    return (
        <>
            {isElectron && <TitleBar />}
            <NotificationBar />
            <Suspense fallback={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - var(--titlebar-height))', color: 'var(--text-tertiary)' }}>加载中...</div>}>
            <Routes>
