...
/** Get API base URL: electron store > localStorage > default */
...
export async function getWsUrl(): Promise<string> {
  if (window.electronAPI?.store) {
    const url = await window.electronAPI.store.get('wsUrl');
    if (url) return url;
  }
  return localStorage.getItem('wsUrl') || 'ws://localhost:8000/ws';
}
...
async function request<T>(url: string, options: RequestInit = {}): Promise<T> {
    ...
    const res = await fetch(`${serverUrl}${url}`, { ...options, headers });
...
export function uploadFileWithProgress(
    ...
    const serverUrl = localStorage.getItem('serverUrl') || DEFAULT_SERVER_URL;
...
export const fileApi = {
    ...
    downloadUrl: (agentId: string, path: string, options?: { inline?: boolean }) => {
        ...
        return `${serverUrl}/agents/${agentId}/files/download?${params.toString()}`;
    },
};
