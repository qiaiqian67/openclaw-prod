...
Canonical project rules are described in `AGENTS.md`. Read it at the start of work. For design/implementation guidance, check `CONTRIBUTING.md`.

The architecture reference document is `ARCHITECTURE_SPEC_EN.md` (if present in the repo).
...
If running on Windows, set `PYTHONUTF8=1` to avoid encoding errors, and set `NO_PROXY=*` if you have a system proxy that should not intercept LLM API calls.
...
`client/` 目录下有独立的 Electron 桌面客户端，复用前端代码，连接自托管后端服务。

详见 `client/DEVELOPMENT.md`。

### 构建命令
```bash
cd client
npm install
npm run electron:dev   # 开发测试
npm run electron:build  # 构建可执行文件
```

### 特性
- 登录页可配置服务器地址（API / WebSocket / 更新服务器）
- 支持私有更新服务器（留空则用 GitHub Releases）
- HashRouter 兼容 Electron file:// 协议
- 自动更新 + 版本检查
